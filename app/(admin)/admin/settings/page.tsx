    ...
    >
      <div className="space-y-4">
        <section className="card overflow-hidden">
          <h2 className="px-4 py-3 text-sm font-bold text-ink border-b border-line flex items-center gap-2">
          ...
          <ul className="divide-y divide-line">
            {((admins || []) as { id: string; name: string; email: string; created_at: string }[]).map(a => (
              <li key={a.id} className="px-4 py-2.5 flex items-center gap-3 text-xs">
                ...
                <span className="text-muted tabular-nums">desde {formatDateBR(a.created_at)}</span>
              </li>
            ))}
            {(admins || []).length === 0 && <li className="px-4 py-8 text-center text-xs text-muted">Nenhum administrador encontrado.</li>}
          </ul>
