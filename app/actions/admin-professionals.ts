 ...
 */
export async function impersonateAction(professionalId: string): Promise<{ success: boolean; error?: string }> {
  try {
    const admin = await assertAdmin();
    ...
    const session: SessionData = {
      ...
      exp: Date.now() + IMPERSONATION_MINUTES * 60_000,
    };
    ...
    await logAdminAction({
      ...
      after: { brand: prof.brand_name, minutes: IMPERSONATION_MINUTES },
    });
...
export async function stopImpersonationAction(): Promise<{ success: boolean }> {
  const cookieStore = await cookies();
  cookieStore.delete(PRO_COOKIE_NAME);
  await logAdminAction({ action: 'professional.impersonate.stop', entityType: 'professional' });
  return { success: true };
}
