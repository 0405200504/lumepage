...
// Linha de estado guardada na própria whatsapp_conversations (sem migração),
// no padrão dos registros _debug_*/_health_*. Guarda o último status e quando
// alertamos/recadastramos por último — evita spam de push e dá heartbeat de cura.
// Prefixo _debug_ para ser filtrado da lista de conversas do painel
...
/**
 * Monitor de saúde do bot (Vercel Cron — ver vercel.json).
 * Para cada profissional com bot ativo:
 *  1. Confere se a instância uazapi está conectada.
 *  2. Confere a config do webhook e RE-REGISTRA sozinho se estiver errada/ausente
 *     (e re-afirma periodicamente para resetar circuit-breaker de entrega da uazapi).
 *  3. Manda notificação push para a profissional quando há algo que exige ação
 *     humana (instância desconectada) — com dedupe para não spammar.
 *
 * Acionável manualmente: GET /api/whatsapp/health com Authorization: Bearer <CRON_SECRET>.
 */
export async function GET(req: NextRequest) {
  ...
  for (const s of allSettings) {
    ...
    try {
      ...
      // 3. Auto-cura: re-registra se quebrado, ou re-afirma periodicamente (heartbeat
      //    anti circuit-breaker — a uazapi pode parar de entregar mesmo com config ok).
      let healed = false;
      ...
      // 4. Problema que exige ação humana
      ...
      // 5. Alerta com dedupe (edge-triggered + re-alerta a cada 6h se persistir)
      let pushed = false;
      if (problem) {
        ...
        // Recuperou após estar com problema → avisa uma vez que voltou.
