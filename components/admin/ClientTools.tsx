  ...
  return (
    <span className="inline-flex items-center gap-1">
      <input value={value} onChange={e => setValue(e.target.value)} autoFocus aria-label="Novo nome"
        className="h-8 px-2 rounded-lg border border-line bg-surface text-xs text-ink w-40" />
      <button type="button" onClick={save} disabled={busy} aria-label="Salvar nome"
        className="p-1.5 rounded-lg text-[color:var(--color-ok)] hover:bg-surface-2">
  ...
  return (
    <div className="card p-4 space-y-3">
      <div className="flex items-center justify-between gap-3">
        <div>
          <p className="text-sm font-bold text-ink tabular-nums">{phoneKey}</p>
          <p className="text-[11px] text-muted">{professionalName} · {clients.length} cadastros</p>
        </div>
        <button type="button" onClick={merge} disabled={busy}
          className="inline-flex items-center gap-1.5 h-9 px-3 rounded-xl bg-forest hover:bg-forest-hover text-white text-xs font-bold disabled:opacity-50 transition-colors">
      ...
      <ul className="space-y-1.5">
        {clients.map(c => (
          <li key={c.id}>
            <label className="flex items-center gap-2.5 px-3 py-2 rounded-xl border border-line hover:bg-surface-2 cursor-pointer text-xs">
              ...
              <span className="flex-1 min-w-0">
                ...
                <span className="block text-[11px] text-muted tabular-nums">{c.whatsapp} · {c.visits} visita(s) · desde {formatDateBR(c.createdAt)}</span>
              </span>
              {primary === c.id && <span className="text-[10px] font-bold uppercase text-accent-link">principal</span>}
            </label>
          </li>
        ))}
      </ul>
