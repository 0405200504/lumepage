...
export function AccessPanel({ data }: { data: AccessPanelData }) {
  ...
  return (
    <div className="space-y-4">
      {!data.available && (
        <p className="card px-4 py-3 text-xs text-[color:var(--color-warn)] font-semibold flex items-start gap-2">
      ...
      {cantLogIn && (
        <p className="card px-4 py-3 text-xs text-[color:var(--color-bad)] font-semibold flex items-start gap-2">
          <AlertTriangle className="h-4 w-4 shrink-0 mt-px" aria-hidden />
          {data.hasAuthUser
      ...
      <section className="card overflow-hidden">
        <h2 className="px-4 py-3 text-sm font-bold text-ink border-b border-line">Credenciais</h2>
        <dl className="divide-y divide-line text-xs">
          <Row label="E-mail de login" hint="é este que ela digita para entrar">
            ...
            <button type="button" onClick={() => setEmailDialog(true)} disabled={!data.hasAuthUser}
              className="text-[11px] font-bold text-accent-link hover:underline disabled:opacity-40 disabled:no-underline">
              Alterar
            </button>
          </Row>
          ...
          <Row label="Último acesso">
            <span className="text-ink font-semibold tabular-nums">{formatDateTimeBR(data.lastSignInAt, 'nunca acessou')}</span>
          ...
          <Row label="De onde" hint="do último acesso registrado por nós">
            <span className="text-muted tabular-nums">{data.lastIp ?? '—'}</span>
            <span className="text-muted">{deviceOf(data.lastDevice)}</span>
          </Row>

          <Row label="Acessos nos últimos 30 dias">
            <span className="text-ink font-semibold tabular-nums">{data.signIns30d}</span>
          </Row>
          ...
          <Row label="Sessões ativas" hint="mais de uma pode ser conta compartilhada">
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 text-[11px] font-bold text-accent-link hover:underline disabled:opacity-40 disabled:no-underline"
            >
          ...
          <Row label="E-mail confirmado">
            {data.emailConfirmedAt
              ? <span className="text-muted tabular-nums">{formatDateBR(data.emailConfirmedAt)}</span>
      ...
      <section className="card overflow-hidden">
        <div className="px-4 py-3 border-b border-line">
          <h2 className="text-sm font-bold text-ink">Ações de acesso</h2>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <section className="card overflow-hidden">
          <h2 className="px-4 py-3 text-sm font-bold text-ink border-b border-line">Últimas entradas</h2>
          {data.history.length === 0 ? (
            ...
            <ul className="divide-y divide-line">
              {data.history.map(h => (
                <li key={h.id} className="px-4 py-2.5 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs">
                  <span className="tabular-nums text-muted w-32 shrink-0">{formatDateTimeBR(h.createdAt)}</span>
                  <Badge tone={h.success ? 'neutral' : 'bad'}>{EVENT_LABEL[h.method] ?? h.method}</Badge>
                  {!h.success && <span className="text-[color:var(--color-bad)] font-semibold">falhou</span>}
                  {h.impersonatedBy && <span className="text-muted truncate">por {h.impersonatedBy}</span>}
                  <span className="ml-auto text-muted tabular-nums" title={h.userAgent ?? undefined}>{h.ip ?? '—'}</span>
                </li>
              ))}
            </ul>
          )}
        ...
        <section className="card overflow-hidden">
          <h2 className="px-4 py-3 text-sm font-bold text-ink border-b border-line">Ações de suporte nesta conta</h2>
          {data.supportActions.length === 0 ? (
            <p className="px-4 py-8 text-center text-xs text-muted">Nenhuma ação de acesso executada aqui.</p>
          ) : (
            <ul className="divide-y divide-line">
              {data.supportActions.map(a => (
                <li key={a.id} className="px-4 py-2.5 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs">
                  <span className="tabular-nums text-muted w-32 shrink-0">{formatDateTimeBR(a.createdAt)}</span>
  ...
  return (
    <div className="px-4 py-2.5 flex flex-wrap items-center gap-x-3 gap-y-1">
      <dt className="w-44 shrink-0">
        ...
        {hint && <span className="block text-[10px] text-faint leading-tight">{hint}</span>}
      </dt>
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center px-4" role="dialog" aria-modal="true" aria-label="Alterar e-mail de login">
      ...
      <div className="relative w-full max-w-md card p-5 space-y-4">
        <div>
          <h2 className="text-sm font-bold text-ink">Alterar o e-mail de login</h2>
          <p className="text-xs text-muted mt-0.5">{brandName} entra hoje com <strong className="text-ink">{current}</strong>.</p>
        </div>
